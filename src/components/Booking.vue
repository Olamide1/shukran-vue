  ...
  <div class="wrapper">
    ...
    <div class="central">
      ...
      <div class="uk-grid-small uk-grid-match" uk-grid>
          ...
          -->
          <div @click="chooseTime(_time, index)"  v-for="(_time, index) in availableTimes" :key="index">
            <div :id="`time-${index}`" class="uk-padding-small a-time uk-border-rounded uk-width-auto uk-card uk-card-default uk-card-body uk-grid-margin">
              {{ _time }} {{ index }}
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      fullname: "",
      selectedTimeIndex: -1,
      availableTimes: ["7:00 AM", "7:30 AM","7:00 AM", "7:30 AM","7:00 AM", "7:30 AM","7:00 AM", "7:30 AM","7:00 AM", "7:30 AM","7:00 AM", "7:30 AM",]
    };
  },
  methods: {
    onDayClick(day) {
    ...
    chooseTime(_time, index) {
      ...
      if (this.selectedTimeIndex == -1 || this.selectedTimeIndex == index) {
        ...
        if (this.selectedTimeIndex > -1) {
          ...
          UIkit.toggle(`#time-${this.selectedTimeIndex}`, {
            cls: 'uk-card-primary'
          }).toggle(); // the extra .toggle is a hot fix
        }

        UIkit.toggle(`#time-${index}`);

        this.selectedTimeIndex = index
      }
      

      console.log('selcected time', _time, index, 'this.selectedTimeIndex', this.selectedTimeIndex);
    }
  },
  mounted() {
    UIkit.toggle('#time-0', {
